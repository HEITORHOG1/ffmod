...
public:
    ...
    void DrawHorizontalHealthBar(Vector3 screenPos, float width, float maxHealth, float currentHealth) {
        ...
        DrawBox(clr, 3, Rect(screenPos.X, screenPos.Y, hpWidth, 4.0f));
    }

    void DrawCrosshair(Color clr, Vector3 center, float size = 44) {
        float x = center.X - (size / 80.0f);
        float y = center.Y - (size / 60.0f);
