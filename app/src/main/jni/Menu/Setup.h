...
void startService(JNIEnv *env, jobject ctx){
    jclass native_context = env->GetObjectClass(ctx);
    jclass intentClass = env->FindClass(OBFUSCATE("android/content/Intent"));
    jclass actionString = env->FindClass(OBFUSCATE("hkg/android/hackudogamer/Launcher"));
...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle){
    //Set sub title
    setText(env, title, OBFUSCATE("<b>HACKUDO GAMER</b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("<b><marquee><p style=\"font-size:30\">"
                                     "<p style=\"color:green;\">HACKUDO GAMER</p> | "
                                     "MOD MENU HACKUDO GAMER GRÁTIS - INSCREVA NO CANAL </p>"
                                     "</marquee></b>"));
    ...
    //Toast Example
    Toast(env,ctx,OBFUSCATE("MOD PARA INSCRITOS DO CANAL HACKUDO GAMER"),ToastLength::LENGTH_LONG);
